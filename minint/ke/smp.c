...
NTSTATUS NTAPI KeDetectMpConfiguration(VOID)
{
    ...
    DbgPrint("SMP: Searching for MP configuration table...\n");
    
    /* SAFETY: Limit search to prevent hangs on systems without MP tables */
    /* Modern UEFI systems don't have MP tables, they use ACPI MADT instead */
    
    /* Search for MP floating pointer in EBDA */
    DbgPrint("SMP: Searching EBDA (%08x-%08x)...\n", EBDA_START, EBDA_END);
    for (i = EBDA_START; i <= EBDA_END; i += 16) {
        if (*(PULONG)i == MPFP_SIGNATURE) {
            MpFp = (PMP_FLOATING_POINTER)i;
            DbgPrint("SMP: Found MPFP signature at %p\n", MpFp);
            break;
        }
    }
    
    if (!MpFp) {
        DbgPrint("SMP: Searching BIOS ROM (%08x-%08x)...\n", BIOS_ROM_START, BIOS_ROM_END);
    ...
    if (!MpFp) {
        DbgPrint("SMP: No MP floating pointer found (UEFI system or single-core)\n");
        DbgPrint("SMP: Falling back to single-processor mode\n");
    ...
    if ((MpFp->MpFeatures[0] & 0x80) == 0) {
        ...
        KeActiveProcessors = 1;  /* Only BSP is active initially */
...
/*
 * KeGetProcessorCount - Get number of ONLINE processors
 * Returns popcount of KeOnlineProcessors mask (actual running CPUs)
 */
ULONG NTAPI KeGetProcessorCount(VOID)
{
    KAFFINITY mask = KeOnlineProcessors;
    ULONG count = 0;
    /* Simple popcount - counts set bits in mask */
    while (mask) {
        count += mask & 1;
        mask >>= 1;
    }
...
{
    KAFFINITY Target;
    Target = KeOnlineProcessors & ~(1ULL << KeGetCurrentProcessorNumber());
