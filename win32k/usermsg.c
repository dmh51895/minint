...
/* Scans oldest-to-newest for the first message matching the filter. On a
 * hit, copies it to *Out and, if Remove is set, closes the gap by shifting
 * every older entry forward one slot — real GetMessage/PeekMessage can
 * return a message from the middle of the queue, not just the head. */
static BOOLEAN Win32kMsgQueueFind(ULONG MsgFilterMin, ULONG MsgFilterMax, BOOLEAN Remove, W32K_MSG *Out)
{
    ULONG i, idx;
...
NTSTATUS APIENTRY UserGetMessage(PW32K_MSG Msg, ULONG_PTR hWnd, ULONG MsgFilterMin, ULONG MsgFilterMax)
{
    ...
    UNREFERENCED_PARAMETER(hWnd); /* per-window filtering arrives with Phase 3 */

    if (!Msg) return STATUS_INVALID_PARAMETER;

    for (;;) {
        KeAcquireSpinLock(&g_MsgQueue.Lock, &OldIrql);
        Found = Win32kMsgQueueFind(MsgFilterMin, MsgFilterMax, TRUE, &Local);
        KeReleaseSpinLock(&g_MsgQueue.Lock, OldIrql);
...
NTSTATUS APIENTRY UserPeekMessage(PW32K_MSG Msg, ULONG_PTR hWnd, ULONG MsgFilterMin, ULONG MsgFilterMax, ULONG RemoveMsg)
{
    ...
    KeAcquireSpinLock(&g_MsgQueue.Lock, &OldIrql);
    Found = Win32kMsgQueueFind(MsgFilterMin, MsgFilterMax,
...
NTSTATUS APIENTRY UserTranslateMessage(PW32K_MSG Msg, ULONG Flags)
{
    ...
    DbgPrint("WIN32K: UserTranslateMessage: VK 0x%02X -> '%c' (0x%04X)\n",
             (ULONG)Msg->wParam, ch, CharMsg);
