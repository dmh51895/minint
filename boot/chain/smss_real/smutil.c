                    ...
                    OUT PVOID *PrivilegeState)
{
    ...
    *PrivilegeState = NULL;
    ...
    /* Open our token */
    Status = NtOpenProcessToken(((HANDLE)(LONG_PTR)-1),
                                0x0020 | 0x0008,
                                &State->TokenHandle);
                    ...
                    OUT PUNICODE_STRING Arguments)
{
    ...
    if (!SmpSystemRoot.Length)
    {
        ...
        RtlInitUnicodeString(&SmpSystemRoot, ((PKUSER_SHARED_DATA)SharedUserData)->NtSystemRoot);
...
SmpQueryRegistrySosOption(VOID)
{
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("SMSS: Cannot open control key (Status 0x%x)\n", Status);
        return FALSE;
    }
    ...
    {
        DbgPrint("SMSS: Cannot query value key (Type %lu, Status 0x%x)\n",
