...
SmpCreatePagingFileDescriptor(IN PUNICODE_STRING PageFileToken)
{
    ...
    do
    {
        ...
        ListDescriptor = ((SMP_PAGEFILE_DESCRIPTOR*)((char*)(NextEntry) - offsetof(SMP_PAGEFILE_DESCRIPTOR, Entry)));
...
SmpSearchVolumeDescriptor(IN WCHAR DriveLetter)
{
    ...
    {
        /* Grab the entry */
        Volume = ((SMP_VOLUME_DESCRIPTOR*)((char*)(NextEntry) - offsetof(SMP_VOLUME_DESCRIPTOR, Entry)));
                    ...
                    IN ULONG Priority)
{
    ...
    if (NT_SUCCESS(Status))
    {
        DbgPrint("SMSS:PFILE: NtCreatePagingFile(%wZ, 0x%I64X, 0x%I64X) succeeded\n",
                Name,
                MinSize->QuadPart,
                MaxSize->QuadPart);
    }
    ...
    {
        DbgPrint("SMSS:PFILE: NtCreatePagingFile(%wZ, 0x%I64X, 0x%I64X) failed with %X\n",
                Name,
                MinSize->QuadPart,
                MaxSize->QuadPart,
                Status);
                                ...
                                IN PLARGE_INTEGER MinimumSize)
{
    ...
    if (Volume->Flags & SMP_VOLUME_IS_BOOT)
    {
        ...
        if (!(Descriptor->Flags & SMP_PAGEFILE_DUMP_PROCESSED))
        {
            ...
            {
                DbgPrint("Queried free space for boot volume `%wC: 0x%I64x'\n",
    ...
    DbgPrint("SMSS:PFILE: Detected size 0x%I64X for future paging file `%wZ'\n",
            PageFileSize,
            &Descriptor->Name);
    DbgPrint("SMSS:PFILE: Free space on volume `%wC' is 0x%I64X\n",
            Volume->DriveLetter,
    ...
    DbgPrint("SMSS:PFILE: min 0x%I64X, max 0x%I64X, real min 0x%I64X\n",
    ...
    if (Descriptor->ActualMinSize.QuadPart < MinimumSize->QuadPart)
    {
        ...
        DbgPrint("SMSS:PFILE: Failing for min 0x%I64X, max 0x%I64X, real min 0x%I64X\n",
                Descriptor->ActualMinSize.QuadPart,
                Descriptor->ActualMaxSize.QuadPart,
                MinimumSize->QuadPart);
...
SmpValidatePagingFileSizes(IN PSMP_PAGEFILE_DESCRIPTOR Descriptor)
{
    ...
    DbgPrint("SMSS:PFILE: Validating sizes for `%wZ' 0x%I64X 0x%I64X\n",
...
SmpCreateVolumeDescriptors(VOID)
{
    ...
    Status = NtQueryInformationProcess(((HANDLE)(LONG_PTR)-1),
                                       15,
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("SMSS:PFILE: Query(15) failed with status %X\n",
                Status);
        return Status;
    }
    ...
    for (Drive = StartChar, DriveDiff = StartChar - L'A'; Drive <= L'Z'; Drive++, DriveDiff++)
    {
        ...
        if (RtlUpcaseUnicodeChar(Drive) ==
            RtlUpcaseUnicodeChar(((PKUSER_SHARED_DATA)SharedUserData)->NtSystemRoot[0]))
        {
            /* Save it */
...
SmpCreatePagingFiles(VOID)
{
    ...
    while (NextEntry != &SmpPagingFileDescriptorList)
    {
        ...
        Descriptor = ((SMP_PAGEFILE_DESCRIPTOR*)((char*)(NextEntry) - offsetof(SMP_PAGEFILE_DESCRIPTOR, Entry)));
