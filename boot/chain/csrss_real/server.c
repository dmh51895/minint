...
PCSR_SERVER_DLL CsrCsrLoadedServerDll[CSR_SERVER_DLL_MAX];
...
CSR_SERVER_DLL_INIT(CsrServerDllInitialization)
{
    ...
    CsrLoadedServerDll->ApiBase = CSRSRV_FIRST_API_NUMBER;
    CsrLoadedServerDll->HighestApiSupported = CsrpMaxApiNumber;
    CsrLoadedServerDll->DispatchTable = CsrServerApiDispatchTable;
    CsrLoadedServerDll->ValidTable = CsrServerApiServerValidTable;
#ifdef CSR_DBG
    CsrLoadedServerDll->NameTable = CsrServerApiNameTable;
#endif
    CsrLoadedServerDll->SizeOfProcessData = 0;
    CsrLoadedServerDll->ConnectCallback = NULL;
    CsrLoadedServerDll->DisconnectCallback = NULL;
                 ...
                 IN ULONG ServerId)
{
    ...
    if (CsrCsrLoadedServerDll[ServerId]) return STATUS_INVALID_PARAMETER;
    ...
    if (NT_SUCCESS(Status))
    {
...
#ifdef CSR_DBG
            DbgPrint("CSRSS: Exception 0x%lx while calling Server DLL entrypoint %Z!%Z()\n",
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            CsrCsrLoadedServerDll[ServerDll->ServerId] = ServerDll;
...
CSR_API(CsrSrvClientConnect)
{
    ...
    *ReplyCode = CsrReplyImmediately;
    ...
    else if (!CsrCsrLoadedServerDll[ClientConnect->ServerId])
    {
    ...
    /* Load the Server DLL */
    ServerDll = CsrCsrLoadedServerDll[ClientConnect->ServerId];
...
CsrSrvCreateSharedSection(IN PCHAR ParameterValue)
{
                        ...
                        0)))
    {
        ...
        NtUnmapViewOfSection(((HANDLE)(LONG_PTR)-1), CsrSrvSharedSectionBase);
        NtClose(CsrSrvSharedSection);
        return STATUS_NO_MEMORY;
    }
...
CsrUnhandledExceptionFilter(IN PEXCEPTION_POINTERS ExceptionInfo)
{
    ...
    ULONG Result = EXCEPTION_EXECUTE_HANDLER;
    BOOLEAN OldValue;
    NTSTATUS Status;
    ...
    DbgPrint("CsrUnhandledExceptionFilter called\n");

    /* Check if a debugger is installed */
    Status = NtQuerySystemInformation(36,
                                      &DebuggerInfo,
                                      sizeof(DebuggerInfo),
                                      NULL);
        ...
        (DebuggerInfo.KernelDebuggerEnabled))
    {
        ...
        if (Result != EXCEPTION_CONTINUE_EXECUTION)
        {
            ...
            if (!NT_SUCCESS(Status))
            {
                DbgPrint("CsrUnhandledExceptionFilter(): RtlAdjustPrivilege(SE_SHUTDOWN_PRIVILEGE) failed, Status = 0x%08lx\n", Status);
                goto NoPrivilege;
            }
...
NoPrivilege:
        /* Just terminate us */
        NtTerminateProcess(((HANDLE)(LONG_PTR)-1),
