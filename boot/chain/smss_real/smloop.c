                        ...
                        IN HANDLE SmApiPort)
{
    ...
    DbgPrint("SmpCreateForeignSession: foreign sessions not supported\n");
...
SmpSessionComplete(IN PSM_API_MSG SmApiMsg,
                   ...
                   IN HANDLE SmApiPort)
{
    DbgPrint("SmpSessionComplete: not supported\n");
    return STATUS_UNSUCCESSFUL;
}
...
SmpTerminateForeignSession(IN PSM_API_MSG SmApiMsg,
                           ...
                           IN HANDLE SmApiPort)
{
    DbgPrint("SmpTerminateForeignSession: not supported\n");
    return STATUS_UNSUCCESSFUL;
}
    ...
    _In_ HANDLE SmApiPort)
{
    ...
    {
        DbgPrint("SMSS: Deferred subsystem load (%wZ) for MuSessionId %u, status=0x%x\n",
            ...
            IN HANDLE SmApiPort)
{
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("SMSS: SmpLoadSubSystemsForMuSession failed with status 0x%08x\n", Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("SMSS: SmpExecuteInitialCommand failed with status 0x%08x\n", Status);
           ...
           IN HANDLE SmApiPort)
{
    ...
    UNREFERENCED_PARAMETER(ClientContext);
    UNREFERENCED_PARAMETER(SmApiPort);
    DbgPrint("SmpStopCsr: CSR shutdown not fully supported\n");
    return STATUS_UNSUCCESSFUL;
}
                           ...
                           IN PSB_API_MSG SbApiMsg)
{
    ...
    if (Accept)
    {
        ...
        else
        {
            ...
            DbgPrint("Rejecting connection due to lack of memory\n");
            Accept = FALSE;
        }
    }
    else
...
SmpApiLoop(IN PVOID Parameter)
{
    ...
    NtQueryInformationProcess(((HANDLE)(LONG_PTR)-1),
                              0,
                              &ProcessInformation,
        ...
        switch (RequestMsg.h.u2.Type)
        {
            ...
            default:
                ...
                if (RequestMsg.ApiNumber >= SmpMaxApiNumber)
                {
                    ...
                    Status = STATUS_INVALID_API_VERSION;
                }
