...
/* Forward declaration: shutdown API from win32k/logon.c */
...
static VOID __attribute__((unused)) DrawAllWindows(VOID);
...
static VOID HandleStartMenuItem(LONG item)
{
    ...
    switch (item) {
        case 0: /* Programs */
            DbgPrint("EXPLORER: Programs submenu (not yet implemented)\n");
            break;
        case 1: /* Documents */
            DbgPrint("EXPLORER: Opening My Documents\n");
            break;
        case 2: /* Settings */
            DbgPrint("EXPLORER: Opening Control Panel (Settings)\n");
            CplOpenApplet(CPL_DISPLAY);
            break;
        case 3: /* Run... */
            DbgPrint("EXPLORER: Run dialog (not yet implemented)\n");
            break;
        case 5: /* Shut down */
            DbgPrint("EXPLORER: Shutting down system\n");
            {
                /* Use the kernel-mode shutdown infrastructure. */
                NTSTATUS NTAPI UserInitiateSystemShutdownW(PCWSTR, PCWSTR, ULONG, BOOL, BOOL);
                UserInitiateSystemShutdownW(NULL, L"Shutdown requested from Start menu",
                                             5, FALSE, FALSE);
            }
            break;
    }
...
static VOID __attribute__((unused)) OpenWindowForIcon(ULONG icon_id)
{
    ULONG win_w = 420;
    ULONG win_h = 320;
            ...
            if (hdc) {
                ...
                if (win) {
                    ...
                    GdiPatBlt((ULONG_PTR)hdc, WIN_BORDER_W, WIN_BORDER_W + WIN_TITLEBAR_H,
                              cx, cy, PATCOPY);
                    GdiExtTextOutW((ULONG_PTR)hdc, ox + 12, oy + 12, 0, 0,
                                   (ULONG_PTR)L"My Computer", 12, 0);
                    GdiExtTextOutW((ULONG_PTR)hdc, ox + 12, oy + 32, 0, 0,
                                   (ULONG_PTR)L"Intel(R) Core(TM) i7-7700K", 29, 0);
                    GdiExtTextOutW((ULONG_PTR)hdc, ox + 12, oy + 50, 0, 0,
                                   (ULONG_PTR)L"8.00 GB RAM", 11, 0);
                    GdiExtTextOutW((ULONG_PTR)hdc, ox + 12, oy + 68, 0, 0,
                                   (ULONG_PTR)L"MinNT 6.1 Build 7601", 21, 0);
                }
            }
    ...
    switch (msg) {
        ...
        case WM_PAINT: {
            ...
            if (hdc) {
                GdiPatBlt((ULONG_PTR)hdc, WIN_BORDER_W, WIN_BORDER_W + WIN_TITLEBAR_H,
                          396, 296, PATCOPY);
                GdiExtTextOutW((ULONG_PTR)hdc, 112, 46, 0, 0,
                               (ULONG_PTR)L"Recycle Bin", 12, 0);
            }
            UserEndPaint((ULONG_PTR)hwnd, NULL);
            break;
        }
...
static LONG_PTR __attribute__((unused)) WndprocDispatcher(HWND hwnd, ULONG msg, ULONG_PTR wParam, LONG_PTR lParam)
{
    switch (msg) {
...
static VOID RegisterWindowClasses(VOID)
{
    ...
    wcscpy(g_ClassName0, L"WndClassMyComputer");
    wcscpy(g_ClassName1, L"WndClassMyDocuments");
    wcscpy(g_ClassName2, L"WndClassRecycleBin");
...
VOID NTAPI ExplorerThread(PVOID Context)
{
    ...
    for (;;) {
        ...
        if (HalpMouseGetEvent(&status, &dx, &dy)) {
            ...
            /* CPL and Admin dialogs take priority when active */
            if (CplIsActive()) {
                if (leftDown && !leftPrev) {
                    CplHandleMouseEvent(MouseX, MouseY, leftDown, leftPrev);
                }
            } else if (AdminIsActive()) {
                if (leftDown && !leftPrev) {
                    AdminHandleMouseEvent(MouseX, MouseY, leftDown, leftPrev);
                }
            } else if (Ad2IsActive()) {
                if (leftDown && !leftPrev) {
                    Ad2HandleMouseEvent(MouseX, MouseY, leftDown, leftPrev);
                }
            } else {
                if (leftDown && !leftPrev) {
        ...
        /* CPL and Admin periodic tick (re-renders active dialog) */
        CplTick();
        AdminTick();
        Ad2Tick();

        /* Secure Attention Sequence (Ctrl+Alt+Del) - bring up the
         * security options / task manager chooser. */
        if (HalpKbdWasSasDetected()) {
            HalpKbdClearSas();
            DbgPrint("EXPLORER: SAS received -> opening task manager\n");
            CplOpenApplet(CPL_TASKMAN);
        }
        ...
        if (HalpKbdHasKey()) {
            ...
            } else if (c == '!') { /* Shift+1 = Display */
                CplOpenApplet(CPL_DISPLAY);
            } else if (c == '@') { /* Shift+2 = Sound */
                CplOpenApplet(CPL_SOUND);
            } else if (c == '#') { /* Shift+3 = Keyboard */
                CplOpenApplet(CPL_KEYBOARD);
            } else if (c == '$') { /* Shift+4 = Mouse */
                CplOpenApplet(CPL_MOUSE);
            } else if (c == '%') { /* Shift+5 = Power */
                CplOpenApplet(CPL_POWER);
            } else if (c == '^') { /* Shift+6 = Region/Time */
                CplOpenApplet(CPL_REGIONAL);
            } else if (c == '&') { /* Shift+7 = Accessibility */
                CplOpenApplet(CPL_ACCESSIBILITY);
            } else if (c == '*') { /* Shift+8 = Notifications */
                CplOpenApplet(CPL_NOTIFICATIONS);
            } else if (c == '(') { /* Shift+9 = Privacy */
                CplOpenApplet(CPL_PRIVACY);
            } else if (c == 'i' || c == 'I') {
                CplOpenApplet(CPL_SYSTEM);
            } else if (c == 't' || c == 'T') {
                CplOpenApplet(CPL_TASKMAN);
            } else if (c == 'p' || c == 'P') {
                CplOpenApplet(CPL_PERSONALIZE);
            } else if (c == 's' || c == 'S') {
                CplOpenApplet(CPL_SCREENSAVER);
            } else if (c == 'm' || c == 'M') {
                CplOpenApplet(CPL_STARTMENU);
            } else if (c == 'b' || c == 'B') {
                CplOpenApplet(CPL_TASKBAR);
            } else if (c == 'c' || c == 'C') {
                CplOpenApplet(CPL_TERMINAL);
            } else if (c == 'x' || c == 'X') {
                CplOpenApplet(CPL_THEMES);
            } else if (c == 'g' || c == 'G') {
                CplOpenApplet(CPL_CONTROLLER);
            } else if (c == 'j' || c == 'J') {
                CplOpenApplet(CPL_STEAMINPUT);
            } else if (c == 'w' || c == 'W') {
                CplOpenApplet(CPL_WINE);
            } else if (c == 'e' || c == 'E') {
                AdminOpenApplet(CPL_EVENTVIEW);
            } else if (c == 'v' || c == 'V') {
                AdminOpenApplet(CPL_SERVICES);
            } else if (c == 'm' || c == 'M') {
                AdminOpenApplet(CPL_DEVICEMGR);
            } else if (c == 'n' || c == 'N') {
                AdminOpenApplet(CPL_NETWORK);
            } else if (c == 'r' || c == 'R') {
                AdminOpenApplet(CPL_PRINTERS); /* Run dialog */
            } else if (c == 'f' || c == 'F') {
                Ad2OpenApplet(CPL_DISKMGMT);
            } else if (c == 'g' || c == 'G') {
                Ad2OpenApplet(100); /* Fonts */
            } else if (c == 'h' || c == 'H') {
                Ad2OpenApplet(300); /* Startup */
            } else if (c == 'j' || c == 'J') {
                Ad2OpenApplet(400); /* User Accounts */
            } else if (c == 'k' || c == 'K') {
                Ad2OpenApplet(500); /* Firewall */
            } else if (c == 'l' || c == 'L') {
                Ad2OpenApplet(600); /* Mixer */
            } else if (c == 'p' || c == 'P') {
                Ad2OpenApplet(CPL_PRINTERS);
            }
        }
