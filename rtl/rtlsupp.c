...
NTSTATUS NTAPI NtReplyWaitReceivePort(HANDLE PortHandle, PVOID *ReplyData,
    PPORT_MESSAGE ReplyMessage, PPORT_MESSAGE ReceiveMessage)
{
    PLPC_PORT port;
    NTSTATUS status;
    PVOID body;
    ...
    UNREFERENCED_PARAMETER(ReplyData);
    ...
    /* Poll the message queue briefly */
    for (i = 0; i < 100; i++) {
        KIRQL _irql;
        KeAcquireSpinLock(&port->Lock, &_irql);
        if (port->MsgQueueHead) {
            msg = (PLPC_MESSAGE)port->MsgQueueHead;
            port->MsgQueueHead = (PLPC_PORT)msg;
            KeReleaseSpinLock(&port->Lock, 0);
            gotMessage = TRUE;
            break;
        }
        KeReleaseSpinLock(&port->Lock, 0);
    ...
    if (!gotMessage) {
        /* Wait on the message event for incoming messages */
        status = KeWaitForSingleObject(&port->MsgEvent, Executive, KernelMode, FALSE, NULL);
        if (!NT_SUCCESS(status)) {
            ObDereferenceObject(body);
            return status;
        }

        /* Try to get a message after being signaled */
        KIRQL _irql;
        KeAcquireSpinLock(&port->Lock, &_irql);
        if (port->MsgQueueHead) {
            msg = (PLPC_MESSAGE)port->MsgQueueHead;
    ...
    if (gotMessage && ReceiveMessage) {
        RtlCopyMemory(ReceiveMessage, msg, sizeof(LPC_MESSAGE));
    }
