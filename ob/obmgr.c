...
POBJECT_TYPE LpcPortObjectType = NULL;
...
NTSTATUS NTAPI ObInitSystem(VOID)
{
    ...
    status = ObCreateObjectType(&typeName, 'eloF', NULL, &IoFileObjectType);
    if (!NT_SUCCESS(status)) {
        DbgPrint("OB: Failed to create File object type: 0x%08lx\n", (ULONG)status);
        return status;
    }
    
    /* Create LPC Port object type */
    RtlInitUnicodeString(&typeName, L"LpcPort");
    status = ObCreateObjectType(&typeName, 'troP', NULL, &LpcPortObjectType);
    if (!NT_SUCCESS(status)) {
        DbgPrint("OB: Failed to create LpcPort object type: 0x%08lx\n", (ULONG)status);
        return status;
    }
    
    DbgPrint("OB: object manager online (File, LpcPort types)\n");
