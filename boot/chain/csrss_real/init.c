...
{
    NTSTATUS Status;
    HANDLE hToken, hProcess = ((HANDLE)(LONG_PTR)-1);
    ULONG Length;
    ...
    /* Open our token */
    Status = NtOpenProcessToken(hProcess, 0x0008, &hToken);
    ...
    ProcSd = RtlAllocateHeap(CsrHeap, HEAP_ZERO_MEMORY, 40 + Length);
    if (!ProcSd)
    {
        Status = STATUS_NO_MEMORY;
        goto Quickie;
    ...
    Dacl = (PACL)((ULONG_PTR)ProcSd + 40);

    /* Now create the SD itself */
    Status = RtlCreateSecurityDescriptor(ProcSd, 1);
    ...
    Status = RtlCreateAcl(Dacl, Length, 2);
    if (!NT_SUCCESS(Status))
    {
...
GetDosDevicesProtection(OUT PSECURITY_DESCRIPTOR DosDevicesSd)
{
    ...
    /* Create the SD */
    Status = RtlCreateSecurityDescriptor(DosDevicesSd, 1);
    ...
    InitializeObjectAttributes(&ObjectAttributes,
                               ...
                               NULL,
                               NULL);
    Status = NtOpenKey(&hKey, 0x00020019, &ObjectAttributes);
    if (NT_SUCCESS(Status))
    {
        /* Read the key value */
        ...
        if ((NT_SUCCESS(Status)) && (KeyValuePartialInfo->Type == 4) &&
    ...
    if (ProtectionMode & 3)
    {
        ...
        Status = RtlCreateAcl(Dacl, AclLength, 2);
                Status = RtlAddAccessAllowedAce(Dacl, 2, 0x10000000, SystemSid);
                Status = RtlAddAccessAllowedAce(Dacl, 2, 0x10000000, AdminSid);
                Status = RtlAddAccessAllowedAce(Dacl, 2, 0x10000000, CreatorSid);
    ...
    else
    {
        ...
        Dacl = RtlAllocateHeap(CsrHeap, HEAP_ZERO_MEMORY, AclLength);
                /* Build the ACL and add 3 ACEs */
        Status = RtlCreateAcl(Dacl, AclLength, 2);
                Status = RtlAddAccessAllowedAce(Dacl, 2, 0x80000000 | 0x40000000 | 0x20000000, WorldSid);
                Status = RtlAddAccessAllowedAce(Dacl, 2, 0x10000000, SystemSid);
                Status = RtlAddAccessAllowedAce(Dacl, 2, 0x10000000, WorldSid);
...
CsrCreateSessionObjectDirectory(IN ULONG Session)
{
    ...
    DbgPrint("%ws\\BNOLINKS", SESSION_ROOT);
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSS: NtCreateDirectoryObject failed in "
    ...
    if (Session)
    {
        ...
        DbgPrint("%ws\\%ld\\BaseNamedObjects", SESSION_ROOT, Session);
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSS: NtCreateSymbolicLinkObject failed in "
                "CsrCreateSessionObjectDirectory - status = %lx\n", Status);
        return Status;
    }
    ...
    DbgPrint("%ws\\%ld", SESSION_ROOT, Session);
    RtlInitUnicodeString(&SessionString, SessionBuffer);
                          ...
                          IN PCHAR Arguments[])
{
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSS: CsrCreateSessionObjectDirectory failed (%lx)\n",
                Status);
    ...
    for (i = 1; i < ArgumentCount; i++)
    {
        ...
        if (_stricmp(ParameterName, "ObjectDirectory") == 0)
        {
            ...
            if (SessionId != 0)
            {
                /* Session IDs > 0 not supported in current MinNT — ignore */
            }
            ...
            InitializeObjectAttributes(&ObjectAttributes,
                                       &CsrDirectoryName,
                                       0x00000020 | 0x00000040 | 0x00000010,
                                       NULL,
                                       NULL);
        ...
        else if (_stricmp(ParameterName, "ServerDll") == 0)
        {
            ...
            /* Load it */
            if (CsrDebug & 1) DbgPrint("CSRSS: Loading ServerDll=%s:%s\n", ParameterValue, EntryPoint);
...
CsrInitCsrRootProcess(VOID)
{
    ...
    if (!ProcessData)
    {
        DbgPrint("CSRSRV:%s: RtlAllocateHeap failed (Status=%08lx)\n",
 ...
 * the local account with 0x001F0000.
 *
 * @param LocalSystemSd
...
CsrCreateLocalSystemSD(OUT PSECURITY_DESCRIPTOR *LocalSystemSd)
{
    ...
    /* Allocate a buffer for the Security Descriptor, with SID and DACL */
    SystemSd = RtlAllocateHeap(CsrHeap, 0, 40 + Length);

    /* Set the pointer to the DACL */
    Dacl = (PACL)((ULONG_PTR)SystemSd + 40);

    /* Now create the SD itself */
    Status = RtlCreateSecurityDescriptor(SystemSd, 1);
    if (!NT_SUCCESS(Status)) goto Quit;

    /* Create the DACL for it */
    RtlCreateAcl(Dacl, Length, 2);

    /* Create the ACE */
    Status = RtlAddAccessAllowedAce(Dacl, 2, 0x001F0000, SystemSid);
    if (!NT_SUCCESS(Status)) goto Quit;
...
CsrSbApiPortInitialize(VOID)
{
    ...
    if (CsrDebug & 2) DbgPrint("CSRSS: Creating %wZ port and associated thread\n", &CsrSbApiPortName);
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        Status = RtlCreateUserThread(((HANDLE)(LONG_PTR)-1),
                                     NULL,
                                     TRUE,
                                     0,
                                     0,
                        ...
                        IN PCHAR Arguments[])
{
    ...
    Status = NtQuerySystemInformation(0,
                                      &CsrNtSysInfo,
                                      sizeof(SYSTEM_BASIC_INFORMATION),
                                      NULL);
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSRV:%s: NtQuerySystemInformation failed (Status=0x%08lx)\n",
                __FUNCTION__, Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSRV:%s: CsrSetProcessSecurity failed (Status=0x%08lx)\n",
                __FUNCTION__, Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSRV:%s: CsrInitializeSessions failed (Status=0x%08lx)\n",
                __FUNCTION__, Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSRV:%s: CsrInitializeProcessStructure failed (Status=0x%08lx)\n",
                __FUNCTION__, Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSRV:%s: CsrParseServerCommandLine failed (Status=0x%08lx)\n",
                __FUNCTION__, Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSRV:%s: CsrInitCsrRootProcess failed (Status=0x%08lx)\n",
                __FUNCTION__, Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSRV:%s: CsrApiPortInitialize failed (Status=0x%08lx)\n",
                __FUNCTION__, Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSRV:%s: CsrSbApiPortInitialize failed (Status=0x%08lx)\n",
                __FUNCTION__, Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSRV:%s: SmConnectToSm failed (Status=0x%08lx)\n",
                __FUNCTION__, Status);
        return Status;
    }
#ifdef __REACTOS__
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSRV:%s: NtSetDefaultHardErrorPort failed (Status=0x%08lx)\n",
                __FUNCTION__, Status);
        return Status;
    }
...
{
    DbgPrint("Deprecated API in r55585.\n");
    return;
}
