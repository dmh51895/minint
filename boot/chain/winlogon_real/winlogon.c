...
UpdateTcpIpInformation(VOID)
{
    ...
    lError = RegQueryValueExW(hKey,
                              ...
                              (LPBYTE)pszBuffer,
                              &dwSize);
    if (((lError == ERROR_INSUFFICIENT_BUFFER) || (lError == ERROR_MORE_DATA)) && (dwType == 1))
    {
    ...
    if ((lError == ERROR_SUCCESS) && (dwType == 1))
    {
...
InitKeyboardLayouts(VOID)
{
    ...
    if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Keyboard Layout\\Preload", 0, 0x00020019, &hKey) == ERROR_SUCCESS)
    {
        while (TRUE)
        {
