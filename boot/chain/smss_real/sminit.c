                     ...
                     IN BOOLEAN Flags)
{
    ...
    if (Flags)
    {
        ...
        {
            /* Get each entry */
            RegEntry = ((SMP_REGISTRY_VALUE*)((char*)(NextEntry) - offsetof(SMP_REGISTRY_VALUE, Entry)));
                             ...
                             IN PVOID EntryContext)
{
    ...
    if ((ValueType == 7) || (ValueType == 1))
    {
        ...
        while (*DllName)
        {
            ...
            if (!(NT_SUCCESS(Status)) || (ValueType == 1)) return Status;
...
SmpTranslateSystemPartitionInformation(VOID)
{
    ...
    InitializeObjectAttributes(&ObjectAttributes,
                               ...
                               NULL,
                               NULL);
    Status = NtOpenKey(&KeyHandle, 0x00020019, &ObjectAttributes);
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("SMSS: Cannot open system setup key for reading: 0x%x\n", Status);
        return;
    }
    ...
    NtClose(KeyHandle);
    if (!NT_SUCCESS(Status) ||
        ((PartialInfo->Type != 1) && (PartialInfo->Type != 2)))
    {
        DbgPrint("SMSS: Cannot query SystemPartition value (Type %lu, Status 0x%x)\n",
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("SMSS: Cannot find drive letter for system partition: 0x%x\n", Status);
    ...
    InitializeObjectAttributes(&ObjectAttributes,
                               ...
                               NULL,
                               NULL);
    Status = NtOpenKey(&KeyHandle, 0x000F003F, &ObjectAttributes);
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("SMSS: Cannot open software setup key for writing: 0x%x\n", Status);
        return;
    }
...
SmpCreateSecurityDescriptors(IN BOOLEAN InitialCall)
{
    ...
    Status = RtlCreateAcl(Acl, AclLength, 2);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x10000000, AdminSid);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x10000000, SystemSid);
    ASSERT(NT_SUCCESS(Status));
...
NextAcl:
    ...
    Status = RtlCreateAcl(Acl, AclLength, 2);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x20000000, WorldSid);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x20000000, RestrictedSid);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x10000000, AdminSid);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x20000000 | 0x80000000 | 0x40000000, WorldSid);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x20000000 | 0x80000000 | 0x40000000, RestrictedSid);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x10000000, AdminSid);
    ASSERT(NT_SUCCESS(Status));
...
NotInitial:
    ...
    Status = RtlCreateAcl(Acl, AclLength, 2);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x20000000 | 0x80000000, WorldSid);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x20000000 | 0x80000000, RestrictedSid);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x10000000, AdminSid);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x20000000 | 0x80000000, WorldSid);
    ...
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x10000000, AdminSid);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Acl, 2, 0x10000000, OwnerSid);
    ASSERT(NT_SUCCESS(Status));
    ...
    while (!IsListEmpty(Head))
    {
        ...
        if (Status == STATUS_OBJECT_NAME_EXISTS)
        {
            ...
            if (RegEntry->Value.Length)
            {
                ...
                ObjectAttributes.Attributes &= ~0x00000020;
                Status = NtCreateSymbolicLinkObject(&DirHandle,
                                                    0x001F0001,
                                                    &ObjectAttributes,
                                                    &RegEntry->Value);
                               ...
                               IN PUNICODE_STRING Path)
{
    ...
    while (NextEntry != &SmpKnownDllsList)
    {
        ...
        /* Checksum it */
        Status = LdrVerifyImageMatchesChecksum((HANDLE)((ULONG_PTR)FileHandle | 1), 0);
        if (!NT_SUCCESS(Status))
        {
...
SmpCreateDynamicEnvironmentVariables(VOID)
{
    ...
    InitializeObjectAttributes(&ObjectAttributes,
                               ...
                               NULL,
                               NULL);
    Status = NtOpenKey(&KeyHandle, 0x40000000, &ObjectAttributes);
    if (!NT_SUCCESS(Status))
    {
        /* Bail out on failure */
    ...
    DbgPrint("%u", ProcessorInfo.ProcessorLevel);
    ...
    InitializeObjectAttributes(&ObjectAttributes,
                               ...
                               NULL,
                               NULL);
    Status = NtOpenKey(&KeyHandle2, 0x00020019, &ObjectAttributes);
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("SMSS: Unable to open %wZ - %x\n", &DestinationString, Status);
        NtClose(KeyHandle);
        ...
        ((PartialInfo->Type != 1) && (PartialInfo->Type != 2)))
    {
        NtClose(KeyHandle2);
        NtClose(KeyHandle);
        DbgPrint("SMSS: Unable to read %wZ\\%wZ (Type %lu, Status 0x%x)\n",
    ...
    NtClose(KeyHandle2);
    if (NT_SUCCESS(Status) &&
        ((PartialInfo2->Type == 1) || (PartialInfo2->Type == 2)))
    {
    ...
    switch (ProcessorInfo.ProcessorArchitecture)
    {
        ...
        case PROCESSOR_ARCHITECTURE_INTEL:
            if ((ProcessorInfo.ProcessorRevision >> 8) == 0xFF)
            {
                ...
                DbgPrint("%02x", ProcessorInfo.ProcessorRevision & 0xFF);
                _wcsupr(ValueBuffer);
                break;
            }
        ...
        case PROCESSOR_ARCHITECTURE_IA64:
        case PROCESSOR_ARCHITECTURE_AMD64:
            DbgPrint("%04x", ProcessorInfo.ProcessorRevision);
            break;

        /* And anything else we'll just read the whole revision identifier */
        default:
            DbgPrint("%u", ProcessorInfo.ProcessorRevision);
            break;
    }
    ...
    DbgPrint("%d", BasicInfo.NumberOfProcessors);
    DbgPrint("Setting %wZ to %S\n", &ValueName, ValueBuffer);
    Status = NtSetValueKey(KeyHandle,
                           &ValueName,
                           0,
    ...
    InitializeObjectAttributes(&ObjectAttributes,
                               ...
                               NULL,
                               NULL);
    Status = NtOpenKey(&KeyHandle2, 0x000F003F, &ObjectAttributes);
    if (NT_SUCCESS(Status))
    {
        ...
        {
            DbgPrint("SMSS: Failed to query SAFEBOOT option (Type %lu, Status 0x%x)\n",
                    PartialInfo->Type, Status);
        }
    }
...
SmpProcessFileRenames(VOID)
{
    ...
    if (SmpAllowProtectedRenames)
        DbgPrint("SMSS: FIXME: Handle SFC-protected file renames!\n");
        ...
        if (NT_SUCCESS(Status))
        {
                ...
                Buffer->ReplaceIfExists)
            {
                ...
                else
                {
                    ...
                    BasicInfo.FileAttributes = 0x00000080;
        ...
        OUT PHANDLE ProcessHandle)
{
    ...
    /* Enable hard errors */
    HardErrorMode = TRUE;
    NtSetInformationProcess(((HANDLE)(LONG_PTR)-1),
                            12,
                            &HardErrorMode,
