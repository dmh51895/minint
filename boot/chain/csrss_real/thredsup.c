                          ...
                          IN PCLIENT_ID ClientId)
{
    ...
    {
        /* Get the thread */
        FoundThread = ((CSR_THREAD*)((char*)(NextEntry) - offsetof(CSR_THREAD, HashLinks)));
                         ...
                         IN PCLIENT_ID Cid)
{
    ...
    {
        /* Get Thread Entry */
        FoundThread = ((CSR_THREAD*)((char*)(NextEntry) - offsetof(CSR_THREAD, Link)));
                      ...
                      IN PCLIENT_ID ClientId)
{
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("Failed to query thread times: %lx\n", Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("No known process for %lx\n", ClientId->UniqueProcess);
        return Status;
    }
    ...
    if (!CsrThread)
    {
        DbgPrint("CSRSRV: %s: out of memory!\n", __FUNCTION__);
        CsrUnlockProcess(CsrProcess);
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("Thread duplication failed: %lx\n", Status);
                ...
                IN BOOLEAN HaveClient)
{
    ...
    if (HaveClient)
    {
        ...
        if (!CurrentThread)
        {
            DbgPrint("CSRSRV: %s: invalid thread!\n", __FUNCTION__);
            CsrReleaseProcessLock();
            return STATUS_THREAD_IS_TERMINATING;
        }
    }
                        ...
                        OUT PCSR_THREAD *CsrThread)
{
    ...
    {
        /* Get the Thread */
        CurrentThread = ((CSR_THREAD*)((char*)(NextEntry) - offsetof(CSR_THREAD, HashLinks)));
