...
/*
 * KeInitializeLapicTimer - Initialize Local APIC timer for system clock
 *
 * The LAPIC timer runs off the bus clock (or core crystal clock on newer CPUs)
 * and generates periodic interrupts without requiring IO APIC routing.
 *
 * This replaces the legacy PIT when APIC is enabled.
 *
 * Timer calculation:
 *   - Target: 100Hz (10ms period)
 *   - BUS clock is typically 100MHz (10ns per tick)
 *   - Count = 100MHz / 100Hz = 1,000,000 ticks
 *
 * We calibrate against the PIT to handle variable bus frequencies.
 */

#define LAPIC_TIMER_VECTOR      0x20    /* IRQ0 -> vector 0x20 (same as PIT) */
#define LAPIC_TIMER_HZ          100     /* 100Hz = 10ms */

static ULONG LapicBusClock;             /* Bus clock frequency in Hz */

/*
 * LapicCalibrateTimer - Calibrate LAPIC timer against PIT
 *
 * Uses PIT channel 2 to measure LAPIC ticks in a known time period.
 */
static ULONG LapicCalibrateTimer(VOID)
{
    ULONG initialCount = 0xFFFFFFFF;
    ULONG finalCount;
    ULONG ticksPer10ms;
    
    /* Disable PIT channel 2 gate temporarily */
    WRITE_PORT_UCHAR(0x61, (READ_PORT_UCHAR(0x61) & 0xFD) | 0x01);
    
    /* Program PIT channel 2 for one-shot, 10ms delay
     * PIT frequency = 1193182 Hz
     * 10ms = 0.01s
     * Count = 1193182 * 0.01 = 11931
     */
    WRITE_PORT_UCHAR(0x43, 0xB0);  /* Channel 2, write LSB then MSB, mode 0 */
    WRITE_PORT_UCHAR(0x42, 0x00);  /* LSB = 0 first (will write full count) */
    WRITE_PORT_UCHAR(0x42, 0x2E);  /* MSB = 46 (approx 10ms with 11931 count) */
    
    /* Actually use 11931 for proper timing: */
    WRITE_PORT_UCHAR(0x43, 0xB0);
    WRITE_PORT_UCHAR(0x42, (11931 >> 0) & 0xFF);
    WRITE_PORT_UCHAR(0x42, (11931 >> 8) & 0xFF);
    
    /* Set LAPIC timer to max count, no interrupts yet */
    if (LapicMode == APIC_MODE_X2APIC) {
        KeWriteMsr(MSR_IA32_X2APIC_LVT_TIMER, 0x00010000 | LAPIC_TIMER_VECTOR); /* Masked */
        KeWriteMsr(MSR_IA32_X2APIC_DIV_CONF, 0x0B);  /* Divide by 1 */
        KeWriteMsr(MSR_IA32_X2APIC_INIT_COUNT, initialCount);
    } else {
        LapicWrite(LAPIC_LVT_TIMER, 0x00010000 | LAPIC_TIMER_VECTOR); /* Masked */
        LapicWrite(LAPIC_TIMER_DIV, 0x0B);  /* Divide by 1 */
        LapicWrite(LAPIC_TIMER_INITCNT, initialCount);
    }
    
    /* Start PIT and wait for it to count down */
    WRITE_PORT_UCHAR(0x61, READ_PORT_UCHAR(0x61) | 0x01);  /* Gate high */
    
    /* Wait for PIT to finish (poll status bit) */
    while (!(READ_PORT_UCHAR(0x61) & 0x20)) {
        __asm__ __volatile__("pause");
    }
    
    /* Stop LAPIC timer and read remaining count */
    if (LapicMode == APIC_MODE_X2APIC) {
        KeWriteMsr(MSR_IA32_X2APIC_LVT_TIMER, 0x00010000 | LAPIC_TIMER_VECTOR); /* Masked = stopped */
        finalCount = (ULONG)KeReadMsr(MSR_IA32_X2APIC_CUR_COUNT);
    } else {
        LapicWrite(LAPIC_LVT_TIMER, 0x00010000 | LAPIC_TIMER_VECTOR); /* Masked = stopped */
        finalCount = LapicRead(LAPIC_TIMER_CURRCNT);
    }
    
    ticksPer10ms = initialCount - finalCount;
    
    /* Calculate bus frequency */
    LapicBusClock = ticksPer10ms * 100;  /* ticks per 10ms * 100 = ticks per second */
    
    return ticksPer10ms;
}

/*
 * KeInitializeLapicTimer - Initialize LAPIC timer as system clock
 *
 * Called after APIC is initialized. Sets up periodic timer at 100Hz.
 */
VOID NTAPI KeInitializeLapicTimer(VOID)
{
    ULONG ticksPer10ms;
    KIRQL OldIrql;
    
    DbgPrint("KE: Calibrating Local APIC timer...\n");
    
    /* Raise IRQL to prevent interrupts during calibration */
    OldIrql = KfRaiseIrql(HIGH_LEVEL);
    
    /* Calibrate the timer */
    ticksPer10ms = LapicCalibrateTimer();
    
    KfLowerIrql(OldIrql);
    
    DbgPrint("KE: LAPIC timer calibrated: %u ticks/10ms (bus clock ~%u MHz)\n",
             ticksPer10ms, LapicBusClock / 1000000);
    
    /* NOTE: Interrupt handler (KiClockInterrupt) is connected in irql.c */
    /* Connect to vector 0x20 - the handler is already set up there */
    extern VOID KiConnectLapicTimer(VOID);
    KiConnectLapicTimer();
    
    /* Set up LVT timer: periodic mode, unmasked, vector 0x20 */
    if (LapicMode == APIC_MODE_X2APIC) {
        KeWriteMsr(MSR_IA32_X2APIC_LVT_TIMER, 
                   APIC_LVT_TIMER_PERIODIC | LAPIC_TIMER_VECTOR);
        KeWriteMsr(MSR_IA32_X2APIC_DIV_CONF, 0x0B);  /* Divide by 1 */
        KeWriteMsr(MSR_IA32_X2APIC_INIT_COUNT, ticksPer10ms);
    } else {
        LapicWrite(LAPIC_LVT_TIMER, 
                   APIC_LVT_TIMER_PERIODIC | LAPIC_TIMER_VECTOR);
        LapicWrite(LAPIC_TIMER_DIV, 0x0B);  /* Divide by 1 */
        LapicWrite(LAPIC_TIMER_INITCNT, ticksPer10ms);
    }
    
    DbgPrint("KE: Local APIC timer initialized at 100Hz (vector 0x%02X)\n", 
             LAPIC_TIMER_VECTOR);
}
