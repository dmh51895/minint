...
{
    ApiMessage->ReturnValue = STATUS_UNSUCCESSFUL;
    return TRUE;
}
...
CsrSbApiHandleConnectionRequest(IN PSB_API_MSG Message)
{
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSS: Sb Accept Connection failed %lx\n", Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("CSRSS: Sb Complete Connection failed %lx\n",Status);
    }
...
CsrSbApiRequestThread(IN PVOID Parameter)
{
    ...
    while (TRUE)
    {
        ...
        if (ReceiveMsg.ApiNumber < SbpMaxApiNumber)
        {
            ...
            {
                DbgPrint("CSRSS: %s Session Api called and failed\n",
        ...
        else
        {
            ...
            ReplyMsg->ReturnValue = STATUS_INVALID_API_VERSION;
        }
    }
}
