    ...
    _In_ LPVOID lpParameter)
{
    ...
    hService = OpenServiceW(hSCManager, L"wdmaud", 0x80000000);
    if (!hService)
    {
    ...
    IN OUT PWLSESSION Session)
{
    ...
    if (!(Session->Options & WLX_LOGON_OPT_NO_PROFILE))
    {
        ...
        if (lError == ERROR_SUCCESS)
        {
            ...
            if ((lError == ERROR_SUCCESS) && (dwType == 4) && (cbData == sizeof(dwValue)))
                bNoPopups = !!dwValue;
    ...
    OUT PSECURITY_ATTRIBUTES* ppsa)
{
    ...
    /* Specifically 0x0080 is required. */
    ...
    *ppsa = NULL;
    ...
    // 1.  The Winlogon SID, to be able to give it 0x0080.
    ...
    //     required to 0x0080 (as it's needed to successfully call
