...
#ifdef CSR_DBG
            DbgPrint("API: %d\n", ApiId);
            DbgPrint("CSRSS: %lx (%s) is invalid ApiTableIndex for %Z or is an "
...
CsrApiRequestThread(IN PVOID Parameter)
{
    ...
    while (TRUE)
    {
        ...
        if (CsrDebug & 2)
        {
            DbgPrint("[%02x] CSRSS: [%02x,%02x] - %s Api called from %08x, Process %08x - %08x\n",
                    Teb->ClientId.UniqueThread,
                    ReceiveMsg.Header.ClientId.UniqueProcess,
                    ReceiveMsg.Header.ClientId.UniqueThread,
                    ServerDll->NameTable[ApiId],
        ...
        _SEH2_TRY
        {
            ...
            else if (ReplyCode == CsrReplyDeadClient)
            {
                ...
                if (!NT_SUCCESS(Status2))
                    DbgPrint("CSRSS: Error while replying to the death message, Status 0x%lx\n", Status2);
    ...
    NtTerminateThread(((HANDLE)(LONG_PTR)-2)(), Status);
    return Status;
}
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            if (NT_SUCCESS(Status))
            {
                ...
                {
                    /* Get the Thread */
                    ServerThread = ((CSR_THREAD*)((char*)(NextEntry) - offsetof(CSR_THREAD, Link)));
                    ...
                    IN PCSR_API_MESSAGE ApiMessage)
{
    ...
    _SEH2_TRY
    {
        ...
        if (  ((ULONG_PTR)ClientCaptureBuffer < CsrProcess->ClientViewBase) ||
             (((ULONG_PTR)ClientCaptureBuffer + offsetof(CSR_CAPTURE_BUFFER, PointerOffsetsArray))
        ...
        SizeOfBufferThroughOffsetsArray =
            offsetof(CSR_CAPTURE_BUFFER, PointerOffsetsArray) +
    ...
    while (PointerCount--)
    {
        ...
        if (CurrentOffset != 0)
        {
            ...
            if ( ((CurrentOffset & (sizeof(PVOID)-1)) != 0) ||
                 (CurrentOffset < offsetof(CSR_API_MESSAGE, Data.ApiMessageData)) ||
...
CsrReleaseCapturedArguments(IN PCSR_API_MESSAGE ApiMessage)
{
    ...
    {
#ifdef CSR_DBG
        DbgPrint("*** CSRSS: Took exception during release %x\n", _SEH2_GetExceptionCode());
        if (NtCurrentPeb()->BeingDebugged) DbgBreakPoint();
#endif
        /* Return failure */
