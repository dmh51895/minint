...
NTSTATUS APIENTRY UserReleaseDC(ULONG_PTR hWnd, ULONG_PTR hdc)
{
    ...
    DbgPrint("WIN32K: UserReleaseDC(%p, %p)\n", hWnd, hdc);
    return STATUS_SUCCESS;
}
...
NTSTATUS APIENTRY GdiRestoreDC(ULONG_PTR hdc, INT iSaveLevel)
{
    ...
    UNREFERENCED_PARAMETER(iSaveLevel);
...
NTSTATUS APIENTRY GdiGetTextMetricsW(ULONG_PTR hdc, ULONG_PTR pTm, ULONG cj)
{
    ...
    DbgPrint("WIN32K: GdiGetTextMetricsW(%p) -> height=%d, width=%d\n",
             hdc, pTM->tmHeight, pTM->tmAveCharWidth);
    return STATUS_SUCCESS;
}
...
{
    DbgPrint("WIN32K: GdiMaskBlt not implemented\n");
    return FALSE;
}
...
GdiAlphaBlend(ULONG_PTR hDCDest, LONG xDest, LONG yDest, LONG cxDest, LONG cyDest,
              ...
              ULONG blendfn)
{
    DbgPrint("WIN32K: GdiAlphaBlend not implemented\n");
    return FALSE;
}
...
GdiTransformPoints(ULONG_PTR hdc, ULONG_PTR pPtIn, ULONG_PTR pPtOut, INT cPts, ULONG iMode)
{
    ...
    DbgPrint("WIN32K: GdiTransformPoints(cPts=%d, mode=%d) pass-through\n", cPts, iMode);
    return cPts;
}