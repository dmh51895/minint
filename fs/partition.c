...
NTSTATUS NTAPI PartitionScan(VOID)
{
    ...
    if (is_gpt) {
        ...
        if (__builtin_memcmp(gpt->signature, GPT_SIGNATURE, 8) != 0) {
        ...
        for (ULONG i = 0; i < num_entries; i++) {
            ...
            /* Identify type */
            if (__builtin_memcmp(e->type_guid, GPT_TYPE_MICROSOFT_BASIC, 16) == 0) {
                ...
                if (NT_SUCCESS(s)) {
                    ...
                    if (__builtin_memcmp(bs + 0x52, "FAT32", 5) == 0 ||
                        __builtin_memcmp(bs + 0x36, "FAT32", 5) == 0) {
