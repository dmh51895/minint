...
NTSTATUS NTAPI UsbInitUhciController(ULONG Bus, ULONG Device, ULONG Function)
{
    ...
    if (((ClassRev >> 16) & 0xFFFF) != ((PCI_CLASS_SERIAL << 8) | PCI_SUBCLASS_USB))
        return STATUS_UNSUCCESSFUL;
...
NTSTATUS NTAPI UsbInitSystem(VOID)
{
    ...
    ULONG VendorDevice, ClassCode;
    USHORT VendorId;

    DbgPrint("USB: initializing subsystem...\n");

    /* Initialize xHCI driver first */
    XhciInitialize();

    /* Priority: xHCI (USB 3.x) > EHCI (USB 2.0) > UHCI (USB 1.1) */

    DbgPrint("USB: scanning PCI for xHCI (USB 3.x)...\n");

    for (Bus = 0; Bus < 1; Bus++)
    {
        for (Device = 0; Device < 32; Device++)
        {
            VendorDevice = HalPciReadConfig(Bus, Device, 0, PCI_VENDOR_ID);
            VendorId = (USHORT)(VendorDevice & 0xFFFF);
            if (VendorId == 0xFFFF)
                continue;

            /* Check class code for USB controller */
            ClassCode = HalPciReadConfig(Bus, Device, 0, PCI_CLASS_REVISION);
            UCHAR Class = (ClassCode >> 24) & 0xFF;
            UCHAR SubClass = (ClassCode >> 16) & 0xFF;
            UCHAR ProgIf = (ClassCode >> 8) & 0xFF;

            if (Class != PCI_CLASS_SERIAL || SubClass != PCI_SUBCLASS_USB)
                continue;

            DbgPrint("USB: Found USB controller at %02x:%02x, ProgIf=%02x\n",
                     Bus, Device, ProgIf);

            /* Try xHCI first (USB 3.x) */
            if (ProgIf == XHCI_PCI_PROG_IF_XHCI) {
                DbgPrint("USB: xHCI controller detected (ProgIf=0x30)\n");
    ...
    DbgPrint("USB: No xHCI found, falling back to UHCI...\n");
    ...
    DbgPrint("USB: no USB controller found\n");
    return STATUS_UNSUCCESSFUL;
}
