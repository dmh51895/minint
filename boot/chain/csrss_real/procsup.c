...
CsrLockedDereferenceProcess(PCSR_PROCESS CsrProcess)
{
        ...
        if (LockCount == 0)
    {
        ...
        DbgPrint("Should kill process: %p\n", CsrProcess);
                  ...
                  IN NTSTATUS ExitStatus)
{
    ...
    while (NextEntry != &CsrProcess->ThreadList)
    {
        ...
        CsrThread = ((CSR_THREAD*)((char*)(NextEntry) - offsetof(CSR_THREAD, Link)));
                  ...
                  OUT PLUID Luid)
{
    ...
    if (!hProcess)
    {
        ...
        Status = NtOpenThreadToken(((HANDLE)(LONG_PTR)-2)(),
                                   0x0008,
                                   FALSE,
                                   &hToken);
...
CsrImpersonateClient(IN PCSR_THREAD CsrThread)
{
    ...
    /* Make the call */
    Status = NtImpersonateThread(((HANDLE)(LONG_PTR)-2)(),
                         ...
                         OUT PCSR_PROCESS *CsrProcess)
{
    ...
    {
        /* Get the Process */
        CurrentProcess = ((CSR_PROCESS*)((char*)(NextEntry) - offsetof(CSR_PROCESS, ListLink)));
...
CsrRevertToSelf(VOID)
{
    ...
    Status = NtSetInformationThread(((HANDLE)(LONG_PTR)-2)(),
...
FindProcessForShutdown(IN PLUID CallerLuid)
{
    ...
    {
        /* Get the process */
        CsrProcess = ((CSR_PROCESS*)((char*)(NextEntry) - offsetof(CSR_PROCESS, ListLink)));
        ...
        if (Status == STATUS_ACCESS_DENIED)
        {
            ...
            if (CsrProcess->ThreadCount)
            {
                ...
                CsrThread = ((CSR_THREAD*)((char*)(CsrProcess->ThreadList.Flink) - offsetof(CSR_THREAD, Link)));
