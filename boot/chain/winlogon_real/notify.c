    ...
    _In_ PCWSTR pszKeyName)
{
    ...
    if (GetSystemMetrics(SM_CLEANBOOT) != 0) // TODO: Cache when Winlogon starts
    {
        ...
        lError = RegQueryValueExW(hDllKey,
                                  ...
                                  (PBYTE)&dwValue,
                                  &dwSize);
        if ((lError == ERROR_SUCCESS) && (dwType == 4) && (dwSize == sizeof(dwValue)))
            bSafeMode = !!dwValue;
     ...
     * expansion, independently of the 1 or 2 type. */
    ...
    NOTIFICATION_TYPE Type)
{
         ...
         ListEntry = ListEntry->Flink)
    {
        PNOTIFICATION_ITEM Notification =
            ((NOTIFICATION_ITEM*)((char*)(ListEntry) - offsetof(NOTIFICATION_ITEM, ListEntry)));
